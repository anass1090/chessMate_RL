...
def _load_net(model_name: str = DEFAULT_MODEL_NAME) -> ActorCritic:
    net = ActorCritic()
...
def collect(n_positions: int = N_POSITIONS, seed: int = 42, model_name: str = DEFAULT_MODEL_NAME) -> dict:
    ...
    trunk: nn.Sequential = net.backbone  # Linear→ReLU→Linear→ReLU
    ...
    with torch.no_grad():
        ...
        logits_all, _ = net(obs_t)                      # unpack (logits, values)
        logits_all = logits_all.cpu().numpy()           # (N, 4096)
