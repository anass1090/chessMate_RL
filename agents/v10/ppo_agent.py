...
from environment.v9.kqk_env import KQKEnv
...
def generate_eval_set(n: int = 1000, seed: int = 42,
    ...
    """Return a fixed list of random KQK FENs for held-out evaluation.

    Saves to disk on first call; subsequent calls load the cached file so the
    same positions are always used across training runs.
    """
...
class PPOAgent:
    """
    PPO agent for the KQK environment — v9.

    Clipped surrogate PPO with a shared actor-critic backbone.
    Uses the same persistent worker process architecture as v8.
    Supports both "centrum" and "random" king movement via environment/v7.
    """
    ...
    def gradient_update(self, all_obs: np.ndarray, all_actions: list,
        ...
        # Old log-probs and advantages — computed before any parameter update
        with torch.no_grad():
    ...
    def train(self, n_episodes: int = 100_000, gamma: float = 0.99,
              movement: str = "centrum", log_every: int | None = None,
        ...
        workers = [
            ctx.Process(
                ...
                args=(weight_qs[i], result_q, self.mate_pool,
                      self.curriculum_ratio, episodes_per_worker, gamma, movement),
                daemon=True,
            )
        ...
        try:
            while ep_count < n_episodes:
                ...
                if eval_fens is not None and ep_count % eval_every < n_workers * episodes_per_worker:
                    stats = self.evaluate_on_fens(eval_fens, movement=movement)
                    eval_history.append({
                        "episode":       ep_count,
    ...
    def render_game(self, fen: str | None = None, greedy: bool = True,
                    movement: str = "centrum") -> tuple[list, list]:
        env = KQKEnv(curriculum_ratio=0.0, movement=movement)
        if fen:
    ...
    def animate_game(self, fen: str | None = None, greedy: bool = True,
                     movement: str = "centrum", size: int = 400,
                     interval: int = 600) -> None:
        import chess.svg
        import ipywidgets as widgets
        from IPython.display import display, SVG

        frames, labels = self.render_game(fen=fen, greedy=greedy, movement=movement)
    ...
    def save(self, name: str = "kqk_ppo_v9") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
        torch.save(self.net.state_dict(), path)
        print(f"Saved → {path}")

    def load(self, name: str = "kqk_ppo_v9") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
    ...
    def evaluate(self, n_episodes: int = 50, movement: str = "centrum") -> dict:
        envs     = [KQKEnv(curriculum_ratio=0.0, movement=movement) for _ in range(n_episodes)]
        ...
        print(f"\n--- Eval ({n_episodes} eps, movement={movement}) ---")
        for reason, count in sorted(counts.items()):
            print(f"  {reason:30s}: {count}")
        print(f"  {'mean_steps':30s}: {results['mean_steps']:.1f}")
        print(f"  {'checkmate_rate':30s}: {results['checkmate_rate']*100:.1f}%")

    def evaluate_on_fens(self, fens: list[str], movement: str = "random") -> dict:
        """Run one greedy episode per FEN and return aggregate stats.

        Uses the same fixed position list every call so results are comparable
        across training checkpoints — the held-out eval equivalent of a
        validation loss curve.
        """
        envs     = [KQKEnv(curriculum_ratio=0.0, movement=movement) for _ in fens]
        obs_list = []
...
def _worker_loop(weight_q: mp.Queue, result_q: mp.Queue,
                 ...
                 eps_per_batch: int, gamma: float, movement: str) -> None:
    import sys
    sys.path.insert(0, ".")
    ...
    net = ActorCritic()
    net.eval()
    env = KQKEnv(curriculum_ratio=curriculum_ratio, mate_pool=mate_pool, movement=movement)
