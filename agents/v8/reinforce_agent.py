...
from environment.v7.kqk_env import KQKEnv
...
class ReinforceAgent:
    """
    REINFORCE agent for the KQK environment — v8.
    ...
    Uses the v7 environment (centralizing king opponent).
    """
    ...
    def save(self, name: str = "kqk_reinforce_v8") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
        torch.save(self.net.state_dict(), path)
        print(f"Saved → {path}")

    def load(self, name: str = "kqk_reinforce_v8") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
