...
from environment.v5.kqk_env import KQKEnv
...
class A2CAgent:
    """
    A2C agent for the KQK environment — v5.

    Same as v4 but uses the v5 environment which terminates episodes
    with a -5 penalty when a mate-in-1 is available but not played.
    """
    ...
    def save(self, name: str = "kqk_a2c_v5"):
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
        torch.save(self.net.state_dict(), path)
        print(f"Saved → {path}")

    def load(self, name: str = "kqk_a2c_v5"):
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
