...
from environment.v11.kqk_env import KQKEnv
...
class PPOAgent:
    """
    PPO agent for the KQK environment — v12.

    Like v11 but train() accepts an optional opponent PPOAgent whose weights
    are snapshotted at the start of training and used as a frozen neural
    network opponent (black king). Opponent weights are NOT updated during
    the training run; call train() again with a fresh opponent snapshot to
    iterate.

    When opponent=None the environment falls back to the movement_ratio
    heuristic (same behaviour as v11).
    """
    ...
    def save(self, name: str = "kqk_ppo_v12") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
        torch.save(self.net.state_dict(), path)
        print(f"Saved → {path}")

    def load(self, name: str = "kqk_ppo_v12") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
