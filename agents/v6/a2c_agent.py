...
class A2CAgent:
    """
    A2C agent for the KQK environment — v6.

    Same as v5 but uses the v6 environment which also terminates episodes
    when the queen is left hanging (black king can legally capture it).
    """
    ...
    def save(self, name: str = "kqk_a2c_v6"):
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
        torch.save(self.net.state_dict(), path)
        print(f"Saved → {path}")

    def load(self, name: str = "kqk_a2c_v6"):
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
