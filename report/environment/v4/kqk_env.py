...
class KQKEnv(gym.Env):
    """
    King + Queen vs King endgame environment — v4.

    Extends v3 with a pluggable opponent interface: pass
    opponent_fn(obs, legal_actions) -> action to use any callable (e.g. a
    trained neural network) as the black king. When opponent_fn=None the
    environment falls back to the movement_ratio heuristic from v3.
    """

    def __init__(self, max_steps: int = 200, curriculum_ratio: float = 0.5,
                 mate_pool: list[str] | None = None, n_mate_positions: int = 500,
                 movement_ratio: float = 0.0, opponent_fn=None):
        super().__init__()
    ...
    def opponent_move(self):
        if self.opponent_fn is not None:
            ...
            if not legal:
                return
            action = self.opponent_fn(b.board_to_obs(self.board), legal)
            self.board.push(b.action_to_move(action))
        elif random.random() < self.movement_ratio:
            self._random_move()
        else:
            self._centrum_move()
