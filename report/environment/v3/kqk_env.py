...
class KQKEnv(gym.Env):
    ...
    Adds full reward shaping and a smarter opponent:
      - Missed-mate penalty: -3.0  (agent had a forcing move and didn't play it)
      - Queen-hang penalty:  -5.0  (agent left queen capturable by black king)
      - Step penalty:        -0.15 per white move (urgency to finish quickly)
      - Checkmate reward:    +10.0

    Opponent: "centrum" mode moves the black king toward the board center
    (more realistic play than random), configurable via movement="random".
    Centrality = min(file, 7-file, rank, 7-rank). Higher = more central.
    """
