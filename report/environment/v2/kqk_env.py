...
class KQKEnv(gym.Env):
    ...
    Adds curriculum learning: a fraction of episodes (curriculum_ratio) start
    from a pre-generated pool of mate-in-1 positions, so the agent sees the
    checkmate reward frequently early in training before it has learned to
    reach it from arbitrary positions.

    Observation: float32 (768,) | Action: int [0, 4095] | Rewards: +10 mate, -1 draw, -0.001/step
    """
