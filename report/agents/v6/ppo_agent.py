...
class PPOAgent:
    """
    PPO agent for the KQK environment — v6.

    Clipped surrogate PPO with a shared actor-critic backbone and 31 persistent
    worker processes. The clipped surrogate objective prevents large destabilizing
    policy updates, which was the key improvement over A2C. Supports both
    "centrum" (king moves toward board center) and "random" opponent movement.
    """
    ...
    def save(self, name: str = "kqk_ppo_v6") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
        torch.save(self.net.state_dict(), path)
        print(f"Saved → {path}")

    def load(self, name: str = "kqk_ppo_v6") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
