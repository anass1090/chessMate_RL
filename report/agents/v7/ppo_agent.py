...
from agents.v7.ppo_opponent import PPOOpponent
from environment.v4.kqk_env import KQKEnv
...
class PPOAgent:
    """
    PPO agent for the KQK environment — v7.

    Extends v6 with self-play: train() accepts an optional PPOOpponent whose
    weights are snapshotted at the start of training and used as a frozen neural
    network opponent (black king). Opponent weights are NOT updated during
    the training run; call train() again with a fresh snapshot to iterate.

    When opponent=None the environment falls back to the movement_ratio
    heuristic (centrality blend).
    """
    ...
    def train(
            ...
            opponent: 'PPOOpponent | None' = None,
        ...
        opponent: a PPOAgent whose weights are snapshotted now and used as the
                  frozen black-king opponent for this training run. When None
                  the movement_ratio heuristic is used instead.
        """
        ...
        opponent_weights = opponent._state_dict_np() if opponent is not None else None
        ...
        workers = [
            ctx.Process(
                ...
                args=(weight_qs[i], result_q, self.mate_pool,
                      ...
                      movement_ratio, opponent_weights),
                daemon=True,
            )
    ...
    def render_game(self, fen: str | None = None, greedy: bool = True,
                    ...
                    opponent: 'PPOOpponent | None' = None) -> tuple[list, list]:
        opponent_fn = _make_opponent_fn(opponent._state_dict_np()) if opponent is not None else None
    ...
    def save(self, name: str = "kqk_ppo_v7") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
        torch.save(self.net.state_dict(), path)
        print(f"Saved → {path}")

    def load(self, name: str = "kqk_ppo_v7") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
    ...
    def evaluate(self, n_episodes: int = 50, movement_ratio: float = 0.0,
                 opponent: 'PPOOpponent | None' = None) -> dict:
        opponent_fn = _make_opponent_fn(opponent._state_dict_np()) if opponent is not None else None
        ...
        opp_label = "agent" if opponent is not None else f"ratio={movement_ratio}"
        print(f"\n--- Eval ({n_episodes} eps, opponent={opp_label}) ---")
...
def _make_opponent_fn(weights: dict):
    """Build a callable opponent from a state-dict snapshot (runs in main process)."""
    net = ActorCritic()
    net.load_state_dict({k: torch.from_numpy(v) for k, v in weights.items()})
    net.eval()

    def opponent_fn(obs: np.ndarray, legal: list[int]) -> int:
        with torch.no_grad():
            logits, _ = net(torch.from_numpy(obs).unsqueeze(0))
        action, _ = _sample_action(logits.squeeze(0), legal)
        return action
...
def _worker_loop(weight_q: mp.Queue, result_q: mp.Queue,
    ...
    # Build frozen opponent network inside the worker
    opponent_fn = None
    if opponent_weights is not None:
        opp_net = ActorCritic()
        opp_net.load_state_dict({k: torch.from_numpy(v) for k, v in opponent_weights.items()})
        opp_net.eval()

        def opponent_fn(obs: np.ndarray, legal: list[int]) -> int:
            with torch.no_grad():
                logits, _ = opp_net(torch.from_numpy(obs).unsqueeze(0))
            action, _ = _sample_action(logits.squeeze(0), legal)
            return action

    env = KQKEnv(curriculum_ratio=curriculum_ratio, mate_pool=mate_pool,
                 movement_ratio=movement_ratio, opponent_fn=opponent_fn)
