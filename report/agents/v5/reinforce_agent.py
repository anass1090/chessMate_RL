...
class ReinforceAgent:
    """
    REINFORCE agent for the KQK environment — v5.

    Pure policy gradient with batch-level return normalization and entropy
    regularization. Uses persistent worker processes (31 CPUs) for parallel
    episode collection, which was the key change enabling the ~88% checkmate
    rate achieved with long training runs.
    """
    ...
    def save(self, name: str = "kqk_reinforce_v5") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
        torch.save(self.net.state_dict(), path)
        print(f"Saved → {path}")

    def load(self, name: str = "kqk_reinforce_v5") -> None:
        path = os.path.join(self.MODEL_DIR, f"{name}.pt")
