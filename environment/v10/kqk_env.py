...
class KQKEnv(gym.Env):
    """
    King + Queen vs King endgame environment — v10.

    Like v9 but opponent movement is controlled by movement_ratio (float)
    instead of a fixed string. movement_ratio=0.0 means always centrum,
    movement_ratio=1.0 means always random, anything in between is sampled
    per step.
    """
    ...
    def opponent_move(self):
        if random.random() < self.movement_ratio:
            self._random_move()
        else:
            self._centrum_move()
